...
void main() {
	...
	vec4 wp = mul(u_model[0], vtx);
	float freq = 0.1;
	vec4 wave = vec4(0.0, 0.0, 0.0, 0.0);
	wave += vec4(0.0, 0.5 * (sin((wp.x * freq) + uClock.x) 
			+ cos((wp.z * freq) + uClock.x)), 0.0, 0.0);
	wave += vec4(0.0, 0.5 * (sin((wp.x * freq) + uClock.x) * sin((wp.x * freq * 0.123443) + uClock.x * 0.13232)
			+ cos((wp.z * freq) + uClock.x) * cos((wp.z * freq * 0.12442) + uClock.x * 0.147283)),
			0.0, 0.0);
	vtx += wave;
	...
	normal = normalize(mix(normal, wave, 0.1));
