...
void main() {
	#define UVSCROLL_SPEED 0.01
	#define UVTILE 8.0
	vec2 UV0Scroll = vec2(sin(vTexCoord0.x * UVTILE + sin(uClock.x + vTexCoord0.x * 2.0)), 
						cos(vTexCoord0.y * UVTILE + sin(uClock.x + vTexCoord0.y * 2.0))); // vec2(sin(vWorldPos.x + vWorldPos.z * 0.2), 0.0) * 0.1;
	// UV0Scroll = vec2(sin(UV0Scroll.x * UVTILE + sin(uClock.x)), cos(UV0Scroll.y * UVTILE + cos(uClock.x))); // vec2(sin(vWorldPos.x + vWorldPos.z * 0.2), 0.0) * 0.1;
	UV0Scroll *= UVSCROLL_SPEED;
	//
	...
	N.xy = texture2D(uNormalMap, vTexCoord0 + UV0Scroll).xy * 2.0 - 1.0;
