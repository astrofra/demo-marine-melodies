...
void main() {
	...
	// SLOT 1: point/spot light (with optional shadows)
	{
		...
		L /= max(distance, 1e-8);
		float cone_attenuation = LightAttenuation(L, uLightDir[1].xyz, distance, uLightPos[1].w, uLightDir[1].w, uLightDiffuse[1].w);
		float attenuation = cone_attenuation;
		...
		// color += mix(vec3(1.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0), shadow_mix);
	}
	...
	float gamma = 2.2;
	color = SimpleReinhardToneMapping(color, 1.0);
